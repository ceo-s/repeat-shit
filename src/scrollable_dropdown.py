...
class CTkScrollableDropdown(customtkinter.CTkToplevel):
  ...
  def __init__(self, attach, id_: str, x=None, y=None, button_color=None, height: int = 200, width: int = None,
    ...
    if self.command is None:
      ...
      self.command = lambda val: self.attach.set(val) or command(f"{self.id}:{val}")
  ...
  def place_dropdown(self):
    ...
    self.y_pos = self.attach.winfo_rooty() + self.attach.winfo_reqheight() + \
        5 if self.y is None else self.y + self.attach.winfo_rooty()
